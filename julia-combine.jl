...
for configuration in project[:configurations][:values]()
    ...
    for parametervalue in configuration[:parametervalues][:values]()
        message("ParameterValue \"$(parametervalue[:name])\"")
        parameter = parametervalue[:parameter][:lock]()
    ...
    for tensorcomponent in tensortype[:tensorcomponents][:values]()
        ...
        tensorcomponent2 =
            tensortype2[:createTensorComponent](tensorcomponent[:name],
                tensorcomponent[:storage_index], tensorcomponent[:indexvalues])
    end
    outdent()
end
outdent()
...
for manifold in project[:manifolds][:values]()
    ...
    for discretization in manifold[:discretizations][:values]()
        ...
        for discretizationblock in
            ...
            push!(regions, discretizationblock[:region])
            push!(actives, discretizationblock[:active])
        end
        outdent()
        combined_region = RC.ibox(manifold[:dimension])
        for region in regions
            combined_region = combined_region[:bounding_box](region)
        end
        ...
        discretizationblock2[:region] = combined_region
        discretizationblock2[:active] = combined_active
    end
    outdent()
end
outdent()
...
for tangentspace in project[:tangentspaces][:values]()
    ...
    configuration2 =
        get(project2[:configurations], tangentspace[:configuration][:name])
    tangentspace2 =
...
for field in project[:fields][:values]()
    ...
    tangentspace2 = get(project2[:tangentspaces], field[:tangentspace][:name])
    ...
    for discretefield in field[:discretefields][:values]()
        ...
        configuration2 =
            get(project2[:configurations], discretefield[:configuration][:name])
        discretization2 =
...
for coordinatesystem in project[:coordinatesystems][:values]()
    ...
    configuration2 =
        get(project2[:configurations], coordinatesystem[:configuration][:name])
    manifold2 = get(project2[:manifolds], coordinatesystem[:manifold][:name])
    coordinatesystem2 =
...
apl["fclose_degree"] = HDF5.H5F_CLOSE_STRONG
apl["libver_bounds"] = HDF5.H5F_LIBVER_LATEST, HDF5.H5F_LIBVER_LATEST
hfile2 = h5open(filename2, "r+", cpl=cpl, apl=apl)
    ...
    for discretefield2 in field2[:discretefields][:values]()
        ...
        for discretefieldblock2 in
            ...
            for discretefieldblockcomponent2 in
                ...
                lower2 = discretizationblock2[:region][:lower]()
                upper2 = discretizationblock2[:region][:upper]()
                shape2 = discretizationblock2[:region][:shape]()
                message("combined_region=$lower2:$upper2")
                # 16^3 * 8 B = 32 kB
                # chunksize = fill(min(16,i), dsize)
                ...
                for discretefieldblock in
                    ...
                    lower = discretizationblock[:region][:lower]()
                    upper = discretizationblock[:region][:upper]()
                    shape = discretizationblock[:region][:shape]()
                    message("region=$lower:$upper")
                    data = read(hfile, "$path/$name")
                    @assert size(data) == shape

                    # Write into dataset
                    dataset[lower[1]+1:upper[1], lower[2]+1:upper[2],
                        lower[3]+1:upper[3]] =
                        data
                end
                outdent()
