...
void TensorComponent::read(const Silo<DBfile> &file, const string &loc,
  ...
  m_tensortype = tensortype;
  read_attribute(m_storage_index, file, loc, "storage_index");
  read_attribute(m_indexvalues, file, loc, "indexvalues");
}
#endif
...
#ifdef SIMULATIONIO_HAVE_SILO
string TensorComponent::silo_path() const {
  return tensortype()->silo_path() + "tensorcomponents/" +
