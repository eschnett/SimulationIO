...
if(SILO_INCLUDE_DIR)
  foreach(_silo_version_header silo.h)
    if(EXISTS "${SILO_INCLUDE_DIR}/${_silo_version_header}")
      file(STRINGS "${SILO_INCLUDE_DIR}/${_silo_version_header}" silo_version_major_str REGEX "^#define[\t ]+SILO_VERS_MAJ[\t ]+\".*\"")
      file(STRINGS "${SILO_INCLUDE_DIR}/${_silo_version_header}" silo_version_minor_str REGEX "^#define[\t ]+SILO_VERS_MIN[\t ]+\".*\"")
      file(STRINGS "${SILO_INCLUDE_DIR}/${_silo_version_header}" silo_version_patch_str REGEX "^#define[\t ]+SILO_VERS_PAT[\t ]+\".*\"")
      string(REGEX REPLACE "^#define[\t ]+SILO_VERS_MAJ[\t ]+\"([^\"]*)\".*!#define[\t ]+SILO_VERS_MIN[\t ]+\"([^\"]*)\".*!#define[\t ]+SILO_VERS_PAT[\t ]+\"([^\"]*)\".*" "\\1" SILO_VERSION_STRING "${silo_version_major_str}!${silo_version_minor_str}!${silo_version_patch_str}")
